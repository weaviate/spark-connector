...
class UtilsTest extends AnyFunSuite {
  test("Test Weaviate Datatype to Spark Datatype conversion") {
    ...
    assert(Utils.weaviateToSparkDatatype(List("number").asJava) == DataTypes.DoubleType)
    assert(Utils.weaviateToSparkDatatype(List("number[]").asJava) == DataTypes.createArrayType(DataTypes.DoubleType))
